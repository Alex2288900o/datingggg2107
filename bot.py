...
def start(message):
    ...
    bot.send_message(message.chat.id, '{0.first_name}, вас приветсвует ученик 10П класса! С радостью вам представляю анонимный чат-бот школы 2107🏫!\nПеред началом пользования советую ознакомиться с некоторыми правилами:\n1. Мат в нашем чат-боте не приветствуется❌\n2. Обзывательства собеседников, учителей школы или администрации - бан🐖\n3. Различные непристойности оставьте при себе🔞\n4. Получайте удовольствие от пользования нашим чат-ботом✨\n5. По поводу жалоб и предложений писать @alexeypetrakovco❤\nА теперь начнём общаться!  Укажите ваш пол! '.format(message.from_user), reply_markup = markup)
    ...
    if message.chat.type == 'private':
        if message.text == '👥 Поиск собеседника' or message.text == '✏️ Следующий диалог':
            ...
            if db.set_gender(message.chat.id, 'male'):
                ...
                bot.send_message(message.chat.id, '❌ Вы уже указали ваш пол. Обратитесь в поддержку @AlexeyMontash')
...
new_varnew_var = bot.polling(none_stop = True)
